		...
		if ( is_wp_error($result) ) {
			if ( 'already_active' == $result->get_error_code() || 'blog_taken' == $result->get_error_code() ) {
				...
				if ( $signup->domain . $signup->path == '' ) {
					printf(
						...
						network_site_url( 'butterfly.php', 'login' ),
						$signup->user_login,
						$signup->user_email,
						wp_lostpassword_url()
					);
			...
			<?php else: ?>
				<p class="view"><?php
					...
					printf( __( 'Your account is now activated. <a href="%1$s">Log in</a> or go back to the <a href="%2$s">homepage</a>.' ), network_site_url( 'butterfly.php', 'login' ), network_home_url() );
				?></p>
			<?php endif;
		}
	}
