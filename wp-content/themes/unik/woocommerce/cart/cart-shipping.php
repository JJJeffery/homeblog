...
<tr class="shipping">
	<th><?php
		if ( $show_package_details ) {
			printf( __( 'Shipping #%d', 'unik' ), $index + 1 );
		} else {
			_e( 'Shipping and Handling', 'unik' );
		}
	?></th>
	<td>
		...
		<?php elseif ( ! WC()->customer->get_shipping_state() || ! WC()->customer->get_shipping_postcode() ) : ?>
			...
			<?php if ( is_cart() && get_option( 'woocommerce_enable_shipping_calc' ) === 'no' ) : ?>
				...
				<p><?php _e( 'Please use the shipping calculator to see available shipping methods.', 'unik' ); ?></p>

			<?php elseif ( is_cart() ) : ?>

				<p><?php _e( 'Please continue to the checkout and enter your full address to see if there are any available shipping methods.', 'unik' ); ?></p>

			<?php else : ?>

				<p><?php _e( 'Please fill in your details to see available shipping methods.', 'unik' ); ?></p>
		...
		<?php else : ?>
			...
			<?php if ( is_cart() ) : ?>
				...
				<?php echo apply_filters( 'woocommerce_cart_no_shipping_available_html',
					...
					sprintf( __( 'Sorry, shipping is unavailable %s.', 'unik' ) . ' ' . __( 'If you require assistance or wish to make alternate arrangements please contact us.', 'unik' ), WC()->countries->shipping_to_prefix() . ' ' . WC()->countries->countries[ WC()->customer->get_shipping_country() ] ) .
					'</p></div>'
				); ?>
		...
		<?php if ( $show_package_details ) : ?>
			<?php
				...
				echo '<p class="woocommerce-shipping-contents"><small>' . __( 'Shipping', 'unik' ) . ': ' . implode( ', ', $product_names ) . '</small></p>';
			?>
		<?php endif; ?>
	</td>
</tr>
