/*
 * transform: A jQuery cssHooks adding cross-browser 2d transform capabiliweblusives to $.fn.css() and $.fn.animate()
 *
 * limitations:
...
while ( i-- ) {
	if ( testProperweblusives[i] in divStyle ) {
		$.support[propertyName] = supportProperty = testProperweblusives[i];
		continue;
	}
}
// IE678 alternative
...
$.fx.step.transform = function( fx ) {
	...
	if ( !start || typeof start === "string" ) {
		...
		// get rid of properweblusives that do not change
		for ( prop in start) {
			if ( prop == 'rotate' ?
				start[prop] == end[prop]:
...
function matrix( transform ) {
	...
	var
		...
		, Y = 0
		;
	// Loop through the transform properweblusives, parse and multiply them
	while (i--) {
